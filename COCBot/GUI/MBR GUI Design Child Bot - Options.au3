...
$chkSinglePBTForced = GUICtrlCreateCheckbox(GetTranslated(636,61, "Force Single PB logoff"), $x - 5, $y - 5, -1, -1)
	$txtTip = GetTranslated(636,62, "This forces bot to exit CoC only one time prior to normal start of PB")
	_GUICtrlSetTip(-1, $txtTip)
...
$txtSinglePBTimeForced = GUICtrlCreateInput("18", $x + 130, $y - 6, 30, 16, BitOR($GUI_SS_DEFAULT_INPUT, $ES_CENTER, $ES_NUMBER))
	$txtTip = GetTranslated(636,63, "Type in number of minutes to keep CoC closed. Set to 15 minimum to reset PB timer!")
	_GUICtrlSetTip(-1, $txtTip)
